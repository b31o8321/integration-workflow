...
If the auth model is SELF_AUTH and there are no pre-steps, write "— 无手工前置步骤" for that row.

Note on 输入字段:
- OAuth with subdomain: list the subdomain field as 输入字段; the OAuth token itself is obtained via redirect, not a form field
- Pure OAuth (no subdomain): write "— 无输入字段（OAuth 跳转，无需预填）"
- API Key: list the API Key field as 输入字段 (e.g., "API Key: 平台生成的密钥 [必填]")
- Basic Auth: list username and password as separate 输入字段 entries
...
needs to expose. Standard items to assess — include those that apply based on the platform's API capabilities:
...
- Include Webhook URL display only if Feature 1 (工单 AI 回复) is ✅ or ⚠️ (user must paste
  Shulex's webhook URL into the third-party platform's webhook settings page)
...
- If the only manual step is the Webhook URL row: write just that one row
- If there are no manual steps at all (Feature 1 is ❌ and no other manual steps): write "— 无手工步骤"
...
MANUAL GUIDANCE（需在三方平台手工完成，前端展示操作引导）
  ⚠️ Webhook URL 配置: 前端展示 Shulex Webhook URL，提示用户在三方后台填入  ← Feature 1 可行时才包含此行
  ⚠️ {其他手工步骤，如创建 OAuth App、开启 API 权限等}
  — 无手工步骤    ← Feature 1 不可行且无其他手工步骤时，用此行替换以上所有 ⚠️ 行
...
前端集成:      {从以下模式选择: "OAuth 跳转授权，无手工步骤" / "OAuth 跳转授权 + N 项手工步骤" / "API Key 填写，无手工步骤" / "API Key 填写 + N 项手工步骤" / "多步骤授权 + N 项手工步骤"}
{If MARKETPLACE_APP:}
